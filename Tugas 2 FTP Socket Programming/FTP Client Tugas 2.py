...
def receive_file(s, client_directory, filename):
    try:
...
def send_file(conn, client_directory, filename):
    file_path = os.path.join(client_directory, filename)
...
def main():
    client_directory = input("Masukkan direktori client: ").strip()

    if not os.path.exists(client_directory):
        print("Direktori client tidak ditemukan.")
        return
        ...
        while True:
            ...
            if command.lower() == 'connme':
                ...
                if command.lower() == 'ls':
                    ...
                    receive_file(s, client_directory, filename)
                elif command.lower().startswith("upload"):
                    s.send(command.encode())
                    _, filename = command.split(maxsplit=1)
                    send_file(s, client_directory, filename)
